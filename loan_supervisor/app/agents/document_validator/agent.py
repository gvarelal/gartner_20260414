...
def extract_and_validate_document_fields(application_id: str) -> str:
    """
    Validates and extracts fields from application documents for a given loan application ID.

    Args:
        application_id: The loan application ID (e.g., 'L_0001').

    Returns:
        A formatted string detailing the validated document fields and extraction flags.
    """
    try:
        ...
        validator_data = data.get("document_validator", {})
        return validator_data.get(normalized_id, f"No preconfigured document validation results found for {normalized_id}.")
    except Exception as e:
        return f"Error validating documents: {str(e)}"
...
document_validator_agent = Agent(
    ...
    description="Agent that extracts and validates fields from application documents for a given loan application ID.",
    instruction=_dynamic_instruction_provider,
    tools=[extract_and_validate_document_fields],
